 ...
 * un Indice, de una biblioteca digital. Por ello, su funcionalidad basica
 * es...
 * 1.- Crear el Indice de la biblioteca, representado mediante un Map 
 *     que se implementa mediante una Tabla Hash Enlazada
 * 2.- Recuperar la informacion que exista sobre una palabra dada
 *     (Posting List) en la biblioteca, buscandola en su Indice.
 * 
 * @author (EDA) 
...
public class BuscadorDeLaBibl {
    ...
    protected static String dirLibros =   File.separator 
    ...
    /**
     * Posting: representa la informacion asociada a la aparicion de 
     * un Termino de un Indice en un libro, i.e. el titulo del libro  
     * y (el numero de) la linea donde aparece el Termino.
     * 
     * @author (EDA) 
     * @version (Curso 2019-2020)
     */
    ...
    /** Crea el Buscador de la bibliotecta digital que forman los libros de 
     *  listaLibros. Basicamente, ello supone crear el Indice de la biblioteca 
     *  con, como maximo, maxTerminos. Si no encuentra (el fichero .txt de) un 
     *  libroLanza FileNotFoundException para advertirlo.
     */ 
    /** Crea el Buscador de la bibliotecta digital que forman los libros de 
     *  listaLibros. Basicamente, ello supone crear el Indice de la biblioteca 
     *  con, como maximo, maxTerminos. Si no encuentra (el fichero .txt de) un 
    ...
    public BuscadorDeLaBibl() throws FileNotFoundException { 
        ...
        System.out.println("Creando el Indice de la bibilioteca... " 
                           + listaLibros);
        ...
        System.out.println("Terminos del Indice (talla del Map) = " 
                           + index.talla());
        if (!res) { throw new FileNotFoundException(); }
    }
    
    /** Devuelve true tras actualizar el Buscador de una bibilioteca digital
     *  con los terminos que contiene fichLibro (fichero .txt). Basicamente,  
     *  ello supone actualizar el Indice de la biblioteca con los terminos de 
     *  fichlibro.
     ...
     * asociado a unaPalabra en el Indice de una biblioteca digital (frecuencia de 
     * aparicion del termino y, en su caso, el listado que contiene los titulos y 
     * lineas de los libros de la bibiblioteca en los que aparece, i.e. su Posting List).
     */
    public String buscar(String unaPalabra) { 
        ...
        ListaConPI<BuscadorDeLaBibl.Posting> valor = index.recuperar(clave);
        if (valor == null) {
            res += "La palabra \"" + unaPalabra 
                   + "\" no aparece en ningun libro de esta bibilioteca";
        }
        else {
            res += "Encontradas " + valor.talla() + " apariciones de la palabra \"" 
                   + unaPalabra + "\" en...\n" + valor.toString(); 
        }
        return res;
    }
    
    /** Comprueba si unaPalabra es un termino del Indice de una biblioteca 
     *  digital, i.e. si unaPalabra esta formada por letras del alfabeto 
     *  castellano en minusculas.
     */
    ...
    /** Devuelve una ListaConPI con los terminos de una biblioteca digital que 
     *  solo aparecen una vez en uno de sus libros, o null si no existe ninguno.
     */
    public ListaConPI<Termino> terminosUnicos() {
        /* COMPLETAR */
        LEGListaConPI<Termino> aux = new LEGListaConPI(); 
        ListaConPI<Termino> lista = index.claves();
        //lista = (LEGListaConPI)lista;
        if(lista.talla()==0){
            return /*CORREGIR*/ null;
        }else{
            lista.inicio();
            for(int i=0; i<lista.talla(); i++){
                if(index.recuperar(lista.recuperar()).talla() == 1){
                    aux.insertar(lista.recuperar());
                }
                lista.siguiente();
            }
        }
