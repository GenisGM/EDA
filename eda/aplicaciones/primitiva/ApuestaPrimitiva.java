...
public class ApuestaPrimitiva {
    ...
    protected int posicionDe(NumeroPrimitiva n) {
        ...
        int posicion = 0;
        int talla = combinacion.talla();
        combinacion.inicio();
        while(posicion<talla && !encontrado){
            if(combinacion.recuperar().equals(n)){return posicion;}
            else{
                posicion ++;
