...
public class TablaHash<C, V> implements Map<C, V> {
    ...
    // INCLUIDA LA CONDICION DE REHASHING: invocar al  
    // metodo rehashing() cuando, TRAS insertar una nueva  
    // Entrada en su cubeta (ListaConPI) e incrementar la 
    // talla de la Tabla, factorCarga() > FACTOR_CARGA
    ...
    protected final void rehashing() {
        /* COMPLETAR */
        ListaConPI<EntradaHash<C, V>>[] elviejoArray = elArray;
        this.elArray = new LEGListaConPI[siguientePrimo(2*elviejoArray.length)];
        for(int i=0; i<elArray.length; i++){
            elArray[i]= new LEGListaConPI<EntradaHash<C,V>>();
        }
        talla = 0;
        for(int j=0; j<elviejoArray.length; j++){
            for(elviejoArray[j].inicio(); !elviejoArray[j].esFin(); elviejoArray[j].siguiente()){
                this.insertar(elviejoArray[j].recuperar().clave,elviejoArray[j].recuperar().valor);
            }
        }
    }
    ...
    public final double desviacionTipica() {
        ...
        double numerador = 0;
        for(int i=0; i<elArray.length; i++){
            numerador += Math.pow(elArray[i].talla() - this.factorCarga(),2);
        }
        
        return Math.sqrt(numerador/elArray.length);
    }
    ...
    public final double costeMLocalizar() {
        ...
        for(int i=0; i<elArray.length; i++){
            if(elArray[i].talla() != 0){
                for(int j=1; j<=elArray[i].talla(); j++){
                    res+=j;
                }
            }
        }
